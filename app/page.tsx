...
function TeacherLogin({ onLogin, onBack, onStudent }: { onLogin: () => void; onBack: () => void; onStudent: () => void }) {
  ...
  const submit = (e: React.FormEvent) => { e.preventDefault(); if (email.trim().toLowerCase() !== "teacher@pulseclass.test" || password !== "Pulse@2026") { setError("The username or password is incorrect. Use the test account shown below."); return; } setError(""); onLogin(); };
