...
function session() {
  if (!root.__pulseClassSession) root.__pulseClassSession = {
    ...
    current: 0, participants: [], responses: {}, scores: {}, timerEnd: 0,
...
export async function POST(request: Request) {
  ...
  const state = session();
  if (body.action === "open") Object.assign(state, { lobbyOpen:true, live:false, title:body.title||"Live activity", className:body.className||"", questions:body.questions||[], current:0, responses:{}, scores:{}, timerEnd:0, finished:false });
  ...
  if (body.action === "question") { state.current = Number(body.current || 0); state.responses = {}; state.timerEnd = Date.now() + (state.questions[state.current]?.seconds || 20) * 1000; }
